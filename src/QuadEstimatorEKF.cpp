...
/*
  ...
  if (ekfState(6) < -F_PI) ekfState(6) += 2.f*F_PI;
*/
    Mat3x3F rotation_mat;
    float phi = rollEst, theta = pitchEst;
    rotation_mat(0,0) = 1;
    rotation_mat(0,1) = sin(phi) * tan(theta);
    rotation_mat(0,2) = cos(phi) * tan(theta);
    rotation_mat(1,0) = 0;
    rotation_mat(1,1) = cos(phi);
    rotation_mat(1,2) = -sin(phi);
    rotation_mat(2,0) = 0;
    rotation_mat(2,1) = sin(phi) / cos(theta);
    rotation_mat(2,2) = cos(phi) / cos(theta);
    
    V3F angles_dot = rotation_mat * gyro;
    float predictedPitch = pitchEst + dtIMU * angles_dot.y;
    float predictedRoll = rollEst + dtIMU * angles_dot.x;
    
    ekfState(6) = ekfState(6) + dtIMU * angles_dot.z;
...
VectorXf QuadEstimatorEKF::PredictState(VectorXf curState, float dt, V3F accel, V3F gyro)
{
  ...
  ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
    V3F a_inert = attitude.Rotate_BtoI(accel);
    predictedState(0) = curState(0) + curState(3) * dt;
    predictedState(1) = curState(1) + curState(4) * dt;
    predictedState(2) = curState(2) + curState(5) * dt;
    
    predictedState(3) = curState(3) + a_inert.x * dt;
    predictedState(4) = curState(4) + a_inert.y * dt;
    predictedState(5) = curState(5) + a_inert.z * dt + (- CONST_GRAVITY * dt ) ;
...
MatrixXf QuadEstimatorEKF::GetRbgPrime(float roll, float pitch, float yaw)
{
  ...
  ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
     float theta = pitch, psi = yaw, phi = roll;
